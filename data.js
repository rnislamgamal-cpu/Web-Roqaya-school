...
const Curriculum={
 ...
 lesson(id){for(const subject of SUBJECTS){const lesson=subject.lessons.find(x=>x.id===id);if(lesson)return {subject,lesson};}return null;},
 valid(c){if(!c||!['beginner','practice'].includes(c.track)||!Number.isInteger(c.week)||c.week<0||c.week>11||!Number.isInteger(c.day)||c.day<0||c.day>4||(c.ratings!==undefined&&(typeof c.ratings!=='object'||!c.ratings||Array.isArray(c.ratings))))return false;return Object.entries(c.ratings||{}).every(([key,r])=>(this.lesson(key)||CURRICULUM.assessment.some(a=>a.id===key))&&r&&[1,2,3].includes(r.level)&&Number.isInteger(r.days)&&r.days>=0&&r.days<=2&&/^\d{4}-\d{2}-\d{2}$/.test(r.date));},
 ...
 review(course){return Object.keys(course.ratings||{}).filter(id=>this.lesson(id)&&!this.mastered(course,id));},
